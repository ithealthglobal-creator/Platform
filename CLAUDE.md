...
This is an **IT Modernisation Platform** built to be white-labelled, with IThealth as the current root admin. Tenant companies (admins, partners, customers) each get their own branded experience. The current scope spans the **Admin area**, **Customer Portal**, and the **public website**.

The platform supports a company hierarchy: `admin` (resellers/MSPs) → `customer`/`partner` (end clients). Roles are `admin`, `partner`, and `customer`.
...
- **Branding** is per-company: `useBranding()` provides CSS vars + logo URLs; `BrandingProvider` is in the `(admin)` layout
